...
const FeedBack = ({ name, doctorId }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (doctorResponse.status === 200) {
        const doctor = doctorResponse.data.data.doctor;
  
        // Extract all objects from doctor.feedback into a new array
        const existingFeedback =doctor.feedback; // Ensure feedback is not undefined
  ...
  console.log(updatedFeedback)
        ...
        const updateResponse = await axios.patch(`http://localhost:3000/api/v1/doctors/update-doctor/${doctorId}`, {
